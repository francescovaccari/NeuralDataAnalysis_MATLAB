        ...
        case 'per_frame_events'
            ...
            switch matchOpt
                ...
                case "first&last"
                    ...
                    allStartTrialIdx = find(stateStates == seq(1)); %find all trials that start with the initial state
                    endTrialIdx = find(stateStates == seq(2)); %find all trials that end with the final state

                    for i = 1:numel(endTrialIdx)
                        candidateStartTrialsIdx = allStartTrialIdx(allStartTrialIdx < endTrialIdx(i));
                        [~, myStartIdxIdx] = max(candidateStartTrialsIdx);
                        startTrialIdx = [startTrialIdx candidateStartTrialsIdx(myStartIdxIdx)];
                    end
...
switch eventType
    case 'per_frame_events'
        ...
        for t = 1:numel(StartsOfTrials)
            ...
            penultimateState = prevStates(endTrialIdx(t));
